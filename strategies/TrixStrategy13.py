...
"""
TrixStrategy13 — v13 : v10 + confirmation du croisement TRIX sur 2 barres (anti-whipsaw).
...
class TrixStrategy13(IStrategy):
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        # === Confirmation 2 barres (v13, anti-whipsaw) ===
        # Bar N-2 sous/au-dessus -> Bar N-1 croise -> Bar N confirme que le signal tient.
        cross_long = (
            (dataframe["trix"].shift(1) > dataframe["trix_signal"].shift(1))
            & (dataframe["trix"].shift(2) <= dataframe["trix_signal"].shift(2))
        )
        cross_short = (
            (dataframe["trix"].shift(1) < dataframe["trix_signal"].shift(1))
            & (dataframe["trix"].shift(2) >= dataframe["trix_signal"].shift(2))
        ...
        # --- LONG (v10 + confirmation : barre N toujours au-dessus) ---
        long_cond = [
            cross_long,
            dataframe["trix"] > dataframe["trix_signal"],  # confirmation barre N
            dataframe["trix"] > 0,
        ...
        # --- SHORT (miroir + confirmation : barre N toujours en-dessous) ---
        short_cond = [
            cross_short,
            dataframe["trix"] < dataframe["trix_signal"],  # confirmation barre N
            dataframe["trix"] < 0,
